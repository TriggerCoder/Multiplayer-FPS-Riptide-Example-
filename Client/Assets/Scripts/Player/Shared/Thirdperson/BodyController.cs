...
public class BodyController : MonoBehaviour {
	...
	void LateUpdate () {
		if (gunAimpoint != null)
		{
			...
			chest.rotation *= Quaternion.Slerp(Quaternion.identity, AimDirection.localRotation, 0.25f);
			spine.rotation *= Quaternion.Slerp(Quaternion.identity, AimDirection.localRotation, 0.25f);
			head.rotation = AimDirection.rotation;

			if (!this.player.clientstate.Sprinting && this.player.clientstate.Grounded)
			{
